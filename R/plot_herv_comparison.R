plot_herv_comparison <- function(x,
  ...
  # ------------------------------------------------------------
  # Helper for numeric comparison tables
  # ------------------------------------------------------------

  plot_numeric_table <- function(df,
                                 filename,
                                 title,
                                 max_items = top_n) {
    ...
    p <- ggplot2::ggplot(
      ...
      ggplot2::labs(
        title = title,
        subtitle = paste0(
          "Positive values indicate higher median in ", x$foreground_name,
          "; tied medians may still differ by distribution/mean"
        ),
        x = NULL,
        y = "Delta median",
        fill = NULL
  ...
  # ------------------------------------------------------------
  # 3. HERVarium external evidence enrichment
  # ------------------------------------------------------------

  plot_enrichment_table(
    x$external_evidence_binary_features,
    filename = "external_evidence_binary_enrichment.png",
    title = "HERVarium external evidence enrichment",
    max_items = top_n,
    pseudocount = pseudocount
  )

  plot_numeric_table(
    x$external_evidence_numeric_features,
    filename = "external_evidence_numeric_shifts.png",
    title = "HERVarium external evidence numeric shifts",
    max_items = top_n
  )

  if (!is.null(x$external_evidence_binary_features) &&
      nrow(x$external_evidence_binary_features) > 0) {

    external_binary <- x$external_evidence_binary_features

    internal_binary <- external_binary[
      grepl("^internal_|^has_internal_", external_binary$feature),
    ]

    ltr_binary <- external_binary[
      grepl("^ltr5_|^ltr3_|^has_ltr|^has_any_ltr|^has_both_ltr", external_binary$feature),
    ]

    plot_enrichment_table(
      internal_binary,
      filename = "internal_external_evidence_binary_enrichment.png",
      title = "Internal-region external evidence enrichment",
      max_items = top_n,
      pseudocount = pseudocount
    )
    ...
    plot_enrichment_table(
      ltr_binary,
      filename = "ltr_external_evidence_binary_enrichment.png",
      title = "LTR external evidence enrichment",
      max_items = top_n,
      pseudocount = pseudocount
    )
  }

  if (!is.null(x$external_evidence_numeric_features) &&
      nrow(x$external_evidence_numeric_features) > 0) {

    external_numeric <- x$external_evidence_numeric_features

    internal_numeric <- external_numeric[
      grepl("^internal_|^dnase_|^h3k27ac_|^h3k4me3_|^same_strand_|^opposite_strand_", external_numeric$feature),
    ]

    ltr_numeric <- external_numeric[
      grepl("^ltr5_|^ltr3_|^any_ltr_|^total_ltr_", external_numeric$feature),
    ]

    plot_numeric_table(
      internal_numeric,
      filename = "internal_external_evidence_numeric_shifts.png",
      title = "Internal-region external evidence numeric shifts",
      max_items = top_n
    )

    plot_numeric_table(
      ltr_numeric,
      filename = "ltr_external_evidence_numeric_shifts.png",
      title = "LTR external evidence numeric shifts",
      max_items = top_n
    )
  }
